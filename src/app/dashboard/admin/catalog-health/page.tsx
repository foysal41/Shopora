...
import { AlertTriangle, ArrowRight, FolderOpen, Loader2, Package, Tags, type LucideIcon } from "lucide-react";
...
const AdminCatalogHealthPage = () => {
  ...
  const catalogMetrics: Array<{
    ...
    { label: "Uncategorized products", value: uncategorized, tone: "text-red-600 bg-red-50", Icon: AlertTriangle },
    { label: "Low-stock products", value: lowStock, tone: "text-amber-600 bg-amber-50", Icon: Package },
    { label: "Empty categories", value: emptyCategories, tone: "text-blue-600 bg-blue-50", Icon: FolderOpen },
    { label: "Inactive categories", value: inactiveCategories, tone: "text-slate-600 bg-slate-100", Icon: Tags },
  ];
    ...
    <main className="min-h-screen bg-[#F8FAFC] px-4 py-5 font-['Poppins'] sm:px-6 lg:px-7">
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
           {catalogMetrics.map(({ label, value, tone, Icon }) => <div key={label} className="flex items-center gap-3 rounded-xl border border-[#E8EEEE] bg-white p-4"><div className={`flex h-10 w-10 items-center justify-center rounded-lg ${tone}`}><Icon size={18} /></div><div><p className="text-xs text-[#64748B]">{label}</p><p className="mt-0.5 text-xl font-semibold text-[#0F172A]">{value}</p></div></div>)}
        </section>
