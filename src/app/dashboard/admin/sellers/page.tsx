...
const ManageSellersPage = () => {
  ...
  const loadSellers = async () => {
    try {
      setLoading(true);
      setError("");
      const [users, products] = await Promise.all([getAdminCustomers(), getAdminProducts()]);
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="overflow-hidden rounded-xl border border-[#E8EEEE] bg-white">
          {loading ? <p className="py-16 text-center text-sm text-[#64748B]">Loading sellers...</p> : filteredSellers.length === 0 ? <p className="py-16 text-center text-sm text-[#64748B]">No sellers found.</p> : <div className="divide-y divide-[#EEF2F2]">{filteredSellers.map((seller) => <div key={seller.id} className="flex flex-col gap-4 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
            ...
            <div className="flex flex-wrap items-center gap-2 sm:justify-end"><span className="inline-flex items-center gap-1 rounded-full bg-[#F8FAFC] px-3 py-1 text-xs text-[#475569]"><Package size={13} />{seller.productCount} products</span><span className="rounded-full bg-[#F8FAFC] px-3 py-1 text-xs text-[#475569]">${seller.inventoryValue.toFixed(2)} inventory</span><span className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-medium ${seller.isBlocked ? "bg-red-50 text-red-600" : "bg-green-50 text-green-600"}`}>{seller.isBlocked ? <Ban size={13} /> : <CheckCircle2 size={13} />}{seller.isBlocked ? "Blocked" : "Active"}</span><button type="button" onClick={() => void toggleBlock(seller)} disabled={actionId === seller.id} className="inline-flex cursor-pointer items-center gap-1 rounded-lg border border-[#E2E8F0] px-3 py-2 text-xs font-medium text-[#475569] hover:border-[#0F766E] hover:text-[#0F766E] disabled:cursor-not-allowed disabled:opacity-50">{seller.isBlocked ? <ShieldCheck size={14} /> : <Ban size={14} />}{seller.isBlocked ? "Unblock" : "Block"}</button><button type="button" onClick={() => void removeSeller(seller)} disabled={actionId === seller.id} className="inline-flex cursor-pointer items-center gap-1 rounded-lg border border-red-100 px-3 py-2 text-xs font-medium text-red-500 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"><Trash2 size={14} />Delete</button></div>
          </div>)}</div>}
        </section>
      </div>
    </main>
